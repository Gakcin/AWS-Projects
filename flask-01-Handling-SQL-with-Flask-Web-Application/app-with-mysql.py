...
app.config['MYSQL_DATABASE_HOST'] = ''#'PLEASE WRITE YOUR DATABASE ENDPOINT HERE after RDS MySQL creating, don't forget RDS MySQL Security group MySQL/Auro port 3306 Anywhere + All Traffic / For EC2 don't forget to add HTTP+SSH Anywhere'
app.config['MYSQL_DATABASE_USER'] = '' # Set it to RDS MySQL
app.config['MYSQL_DATABASE_PASSWORD'] = '' # Set it to RDS MySQL
app.config['MYSQL_DATABASE_DB'] = '' # Set it to RDS MySQL
...
if __name__ == '__main__':
   # app.run(debug=True) # it runs localhost
   app.run(host='0.0.0.0', port=80) # it runs over EC2 intance
